function [parameters, LL, ht, VCVrobust, stdpar, pval, VCV, scores, diagnostics] = tarch(epsilon, p, o, q, error_type, tarch_type, startingvals, options, estPars, Ninsample)
...
% Modified on 07/02/2018 by Trung: Provide options to just compute the
% conditional volatility based on provided parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input Checking
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch nargin
    ...
    case 9
        [p,o,q,error_type,tarch_type,startingvals,options]=tarch_parameter_check(epsilon, p, o, q, error_type, tarch_type);
        %fprintf('Parameters are provided, computing ht and exit...\n');
    case 10
        [p,o,q,error_type,tarch_type,startingvals,options]=tarch_parameter_check(epsilon, p, o, q, error_type, tarch_type);
    otherwise
        error('Number of inputs must be between 4 and 11');
end
...
if ~isempty(estPars) && nargout > 3
    error('Parameter VCV is not computed in cased pameters is provided');
end
...
% Modified date 07/02/2018: if number of the in-sample observation is
% provided, adjust the back_casts accordingly since it will effect the
% first value of conditional volatility and afterwads. 
if nargin == 10 
   if tarch_type == 1
   start_ht = mean(abs(epsilon(1:Ninsample)));
   else
   start_ht = mean(epsilon(1:Ninsample).^2);
   end
else
   if tarch_type == 1
   start_ht = mean(abs(epsilon));
   else
   start_ht = mean(epsilon.^2);    
   end
end

if tarch_type==1
    %fepsilon is f(epsilon), as above
    fepsilon   =  [start_ht*ones(m,1) ; abs(epsilon)];
    %fIepsilon is fepsilon*(epsilon<0)
    fIepsilon   =  [0.5*start_ht*ones(m,1) ; abs(epsilon).*(epsilon<0)];
    ...
    %fepsilon is f(epsilon), as above
    fepsilon   =  [start_ht*ones(m,1) ; epsilon.^2];
    %fIepsilon is fepsilon*(epsilon<0)
    fIepsilon   =  [0.5*start_ht*ones(m,1) ; epsilon.^2.*(epsilon<0)];
    % Local back casting
    back_cast_length = max(floor(length(epsilon)^(1/2)),1);
    back_cast_weights = .05*(.9.^(0:back_cast_length ));
    back_cast_weights = back_cast_weights/sum(back_cast_weights);
...
if nargout>3
    ...
    VCV=hess^(-1)/(T-m);
    diagnostics.A = A;
    stdpar = sqrt(diag(VCVrobust));
    tval = parameters./stdpar;
