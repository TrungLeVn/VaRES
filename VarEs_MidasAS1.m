function [estParams,CondQ,CondES,output] = VarEs_MidasAS1(y,varargin)
%VARES Summary of this function goes here
%   Detailed explanation goes here
quantileDefault = 0.05;
periodDefault = 1;
nlagDefault = 100;
callerName = 'VarEs_MidasAS1';
...
if isempty(startPars)
    ...
    betaIni = IniParAL1(QuantEst,yLowFreq,yHighOri,xHighFreq,q,beta2para,numInitialsRand,numInitials,doparallel);
else
    betaIni = startPars';
end
...
parfor i = 1:size(betaIni,1)  
    ...
    %    [estParams(i,:),fval(i,1),exitFlag(i,1)] = patternsearch(@(params) ALdist1(params,yLowFreq,yHighOri,xHighFreq,q,beta2para),betaIni(i,:),[],[],[],[],lb,ub,options);        
    %else
   [estParams(i,:),fval(i,1),exitFlag(i,1)] = fminsearch(@(params) ALdist1(params,yLowFreq,yHighOri,xHighFreq,q,beta2para),betaIni(i,:));
   for ii = 1:REP
     try
      [estParams(i,:),fval(i,1),exitFlag(i,1)]  = fminunc(@(params) ALdist1(params,yLowFreq,yHighOri,xHighFreq,q,beta2para),estParams(i,:),optionsUnc);
     catch
       warning('fminunc does work. Move on to the fminsearch.');
     end
     [estParams(i,:),fval(i,1),exitFlag(i,1)]  = fminsearch(@(params) ALdist1(params,yLowFreq,yHighOri,xHighFreq,q,beta2para),estParams(i,:),options);
     if constrained
     [estParams(i,:),fval(i,1),exitFlag(i,1)]  = fminsearchbnd(@(params) ALdist1(params,yLowFreq,yHighOri,xHighFreq,q,beta2para),estParams(i,:),lb,ub,options); 
     end
       if exitFlag(i,1) == 1
           break
       end
...
[~,CondQ,CondES,LLH] = ALdist1(estParams,yLowFreq,yHighOri,xHighFreq,q,beta2para);
%%
...
parfor r = 1:nsim
    ...
    paramSim(:,r) = fminsearch(@(params) ALdist1(params,yLowFreqSim,yHighOri,xHighFreq,q,beta2para),estParams,options);
end
else
for r = 1:nsim
    ind = randi(nobsEst,[nobsEst,1]);
...
columnNames = {'Coeff','StdErr','tStat','Prob'};
 if beta2para
    rowNames = {'Intercept';'Slope1';'Slope2';'k1';'k2';'phi'};
 else
    rowNames = {'Intercept';'Slope1';'Slope2';'k2';'phi'};
 end
...
function beta = IniParAL1(QuantEst,yLowFreq,yHighOri,xHighFreq,q,beta2para,numInitialsRand,numInitials,doparallel)
...
parfor i = 1:numInitialsRand
    RQfval(i) = ALdist1(InitialParamsVec(i,:),yLowFreq,yHighOri,xHighFreq,q,beta2para);
end
else
...
function [llh,condQ,es,LLH] = ALdist1(params,yLowFreq,yHighOri,xHighFreq,q,beta2para)
intercept = params(1);
slope1 = params(2);
slope2 = params(3);
if beta2para
...
ALdist = ((q-1)./(es)).*exp(((yLowFreq-condQ).*hit)./(q.*es));
