function [BetaHat, VaR, output] = CAViaR_Uni(y,varargin)
...
% Compute the empirical THETA-quantile for y (the in-sample vector of observations)
   ...
   % structure is sensitive to the choice of how many initial observation
...
if ~isempty(BetaHat)
    ...
    VaRHit  = RQobjectiveFunction(BetaHat', 2, model, y, theta, empiricalQuantile);
    VaR = VaRHit(:,1);
    Hit = VaRHit(:,2);
...
%**************************** Start the optimization ******************************************
    if isempty(startPars)
    initialTargetVectors = unifrnd(0.1, 1, nInitialVectors);
    
    RQfval = zeros(nInitialVectors(1), 1);
    if doparallel
     parfor i = 1:nInitialVectors(1)
        RQfval(i) = RQobjectiveFunction(initialTargetVectors(i,:), 1, model,y, theta, empiricalQuantile);
     end
    else
    ...
    parfor i = 1:size(BestInitialCond,1)
        ...
        for it = 1:REP
            try
            [Beta(i,:), fval(i,1), exitflag(i,1)] = fminunc(@(BETA) RQobjectiveFunction(BETA,1, model, y, theta, empiricalQuantile),Beta(i,:),optionsUnc);    
            catch
            ...
            [Beta(i,:), fval(i,1), exitflag(i,1)] = fminsearch(@(BETA) RQobjectiveFunction(BETA,1, model, y, theta, empiricalQuantile),Beta(i,:),options);            
            if constrained
              ...
              %end
               [Beta(i,:), fval(i,1), exitflag(i,1)] = fminsearchbnd(@(BETA) RQobjectiveFunction(BETA,1, model, y, theta, empiricalQuantile),Beta(i,:),lb,ub,options);      
            end
            if exitflag(i,1) == 1
                break
            end
        ...
        for it = 1:REP
            ...
            if constrained
              if sum(Beta(i,:)' > lb) + sum(Beta(i,:)' < ub) ~= (2*length(Beta(i,:))) % If fminsearch violate the bounds, redo the optimization with fmincon
                   [Beta(i,:), fval(i,1), exitflag(i,1)] = fmincon(@(BETA) RQobjectiveFunction(BETA,1, model, y, theta, empiricalQuantile),Beta(i,:),[],[],[],[],lb,ub,[],optionCon);
              end
             %[Beta(i,:), fval(i,1), exitflag(i,1)] = fminsearchbnd(@(BETA) RQobjectiveFunction(BETA,1, model, y, theta, empiricalQuantile),Beta(i,:),lb,ub,options);      
            end
            if exitflag(i,1) == 1
    ...
    if (model == 1)
        ...
        InitialCond = SortedFval(1, 6:8)';
    else
        BestFval         = SortedFval(1, 1);
        BetaHat    = SortedFval(1, 2:5)';
        ExitFlag         = SortedFval(1, 6);
        InitialCond = SortedFval(1, 7:10)';
    end
...
%************************** Compute variables that enter output *****************************
    ...
    [varCov] = VarianceCovariance(BetaHat', model, nobs, y, theta, VaR);
...
if display
    ...
    if model == 1
        rowNames = {'Intercept';'LaggedBeta';'Beta'};
    else
        rowNames = {'Intercept';'LaggedBeta';'Beta(+)';'Beta(-)'};
    end
...
%------------------------------------------------------------------
% VarCov function
function [VCmatrix, D, gradient, BANDWIDTH] = VarianceCovariance(BETA, MODEL, T, y, THETA, VaR)
