function [estParams,condQuantile,output] = MidasQuantile(y,varargin)
...
addParameter(parseObj,'Constrained',constrainedDefault,@(x)validateattributes(x,{'numeric','logical'},{'binary','nonempty'},callerName));
...
if ~isempty(estParams)
    [~,condQuantile] = objFun(estParams,yLowFreq,xHighFreq,q,beta2para);
...
if ~searchFlag
    ...
    [estParams(i,:),fval(i,1),exitFlag(i,1)] = fmincon(@(params) objFunGrad(params,yLowFreq,xHighFreq,q,beta2para),betaIni(i,:),[],[],[],[],lb,ub,[],options);    
else 
    %if exist('patternsearch','file') ~= 0
    %    [estParams(i,:),fval(i,1),exitFlag(i,1)] = patternsearch(@(params) objFun(params,yLowFreq,xHighFreq,q,beta2para),betaIni(i,:),[],[],[],[],lb,ub,options);        
    %else
        [estParams(i,:),fval(i,1),exitFlag(i,1)] = fminsearch(@(params) objFun(params,yLowFreq,xHighFreq,q,beta2para),betaIni(i,:));        
   for ii = 1:REP
       try
       [estParams(i,:),fval(i,1),exitFlag(i,1)]  = fminunc(@(params) objFun(params,yLowFreq,xHighFreq,q,beta2para),estParams(i,:),optionsUnc);
       catch
       warning('fminunc does work. Move on to the fminsearch.');
       end
        [estParams(i,:),fval(i,1),exitFlag(i,1)]  = fminsearch(@(params) objFun(params,yLowFreq,xHighFreq,q,beta2para),estParams(i,:),options);
       if constrained
           [estParams(i,:),fval(i,1),exitFlag(i,1)]  = fminsearchbnd(@(params) objFun(params,yLowFreq,xHighFreq,q,beta2para),estParams(i,:),lb,ub,options);
       end
            if exitFlag(i,1) == 1
                break
            end
...
[fval,condQuantile] = objFun(estParams,yLowFreq,xHighFreq,q,beta2para);
...
output.weights = estParams(2) * midasBetaWeights(nlag,1,estParams(3));
end
end
rng('default')
end
...
function beta = GetIniParams(yLowFreq, xHighFreq, q, nlag,beta2para,numInitialsRand,numInitials,doparallel)
% Randomly sample second parameter of Beta polynomial
nInitalBeta = unifrnd(1,100,[numInitialsRand,1]);
numparams = 3 + beta2para; % number of parameters to estimates
InitialParamsVec = zeros(numInitialsRand,numparams);
nobsEst = size(yLowFreq,1);
ols = zeros(2,numInitialsRand);
...
parfor i = 1:numInitialsRand
    RQfval(i) = objFun(InitialParamsVec(i,:),yLowFreq,xHighFreq,q,beta2para);
end
else
for i = 1:numInitialsRand
    RQfval(i) = objFun(InitialParamsVec(i,:),yLowFreq,xHighFreq,q,beta2para);
...
function [fval,condQuantile] = objFun(params,y,X,q,beta2para)  
% Allocate parameters
intercept = params(1);
slope = params(2);
if beta2para
...
%-------------------------------------------------------------------------
% Local function: the objective function
function [fval,condQuantile] = objFunCapital(params,y,X,q,beta2para)
...
function [fval,Gradient] = objFunGrad(params,y,X,q,beta2para)

fval = objFun(params,y,X,q,beta2para);
if nargout == 1    
    return
end

fun = @(params) objFunCapital(params,y,X,q,beta2para);
x = params;
...
function [yLowFreqSim,CondQsim] = GetSim(params,xHighFreqSim,beta2para,ResidSim)
...
weights = midasBetaWeights(nlag,k1,k2)';
% Conditional quantile
CondQsim = intercept + slope .* (xHighFreqSim * weights);
