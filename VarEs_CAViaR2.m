    function [estParams,CondQ,CondES,output] = VarEs_CAViaR2(y,varargin)
%VARES Summary of this function goes here
%   Detailed explanation goes here
quantileDefault = 0.05;
periodDefault = 1;
nlagDefault = 0;
callerName = 'VarEs_CAViaR2';
...
function beta = IniParAL2(QuantEst,mu,y, Regressor, theta, empiricalQuantile,numInitialsRand,numInitials,model,doparallel)
...
parfor i = 1:numInitialsRand
    RQfval(i) = ALdist2(InitialParamsVec(i,:),mu,y,Regressor,theta,model,empiricalQuantile);
end
else
...
function [llh,VaR,es] = ALdist2(Params,mu,y,X,THETA,MODEL,empiricalQuantile)
%*****************************************************************************************
if ~isempty(X)
if MODEL == 1
    BETA = Params(1:4); 
...
es = (1 + exp(phi)).*VaR;
hit = THETA - (y<=VaR);
ALdistLog = log(((1-THETA)./(mu - es)).*exp(((VaR-y).*hit)./(THETA.*(mu-es))));
